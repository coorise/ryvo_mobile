...
/// About dialog with installed version info and manual update check.
        ...
        child: Column(
          ...
          children: [
            if (_loadingInfo)
              ...
              _InfoRow(label: 'Version', value: 'v${pkg.version} (build $build)'),
              const SizedBox(height: 8),
              _InfoRow(label: 'Package', value: pkg.packageName),
              const SizedBox(height: 8),
              _InfoRow(label: 'Environment', value: AppUpdate.deployLabel(pkg)),
            ],
            if (_checking) ...[
            ...
            if (_checkMessage != null) ...[
              ...
              Text(
                ...
                style: Theme.of(context).textTheme.bodyMedium?.copyWith(
                      color: _updateAvailable ? Theme.of(context).colorScheme.primary : null,
                      fontWeight: _updateAvailable ? FontWeight.w600 : null,
                    ),
              ),
            ],
            if (_checkError != null) ...[
            ...
            if (_latest != null) ...[
              ...
              _InfoRow(label: 'Latest tag', value: _latest!.tagName),
              if (_updateAvailable) ...[
                const SizedBox(height: 8),
                _InfoRow(
                  label: 'Available',
                  value: 'v${_latest!.version} (build ${_latest!.buildNumber})',
                ),
              ],
              if (_latest!.body.trim().isNotEmpty) ...[
                const SizedBox(height: 16),
                Text('Release notes', style: Theme.of(context).textTheme.titleSmall),
                const SizedBox(height: 8),
                Text(_latest!.body.trim()),
              ],
            ],
          ],
        ),
