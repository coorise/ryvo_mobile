#!/usr/bin/env bash
# Run Ryvo admin on iOS simulator or device (hot reload). Requires macOS + Xcode.
# Usage: ./run_dev_ios.sh [--local|--dev|--prod] [--remote-updates|--no-remote-updates] [flutter run args…]
set -euo pipefail
